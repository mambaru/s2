...
use S2\S2EdgeIndex\S2EdgeIndex_1;
use S2\S2EdgeUtil\EdgeCrosser;
use S2\S2EdgeUtil\WedgeContains;
use S2\S2EdgeUtil\WedgeRelation;
...
class S2Loop implements S2Region
{
    ...
    */
    public function vertex($i) {

        return $this->vertices[$i >= count($this->vertices) ? $i - count($this->vertices) : $i];
    }
    ...
    */
    public function containsNested(S2Loop $b) {
        if (!$this->bound->contains($b->getRectBound())) {
            return false;
        }
    ...
    * // S2Region interface (see {@code S2Region} for details):
     ...
     * edge lookups.
     */
    private final function getEdgeIterator(int $expectedQueries) {
        if ($this->index == null) {
            $this->index = new S2EdgeIndex_1();
     ...
     */
    private function findVertex(S2Point $p) {
        if ($this->vertexToIndex) {
            for ($i = 1; $i <= $this->numVertices; $i++) {
                $this->vertexToIndex[$i] = $this->vertex($i);
            }
        }

        $index = $this->vertexToIndex[$p];
        if ($index == null) {
            return -1;
        } else {
            return $index;
     ...
     */
    private function checkEdgeCrossings(S2Loop $b, WedgeRelation $relation) {
