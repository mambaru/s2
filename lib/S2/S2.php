...
class S2 {
    ...
    public static function posToOrientation($position) {
        if (!(0 <= $position && $position < 4)) throw new \RuntimeException();
...
// 0 1 2 3
    ...
    public static function posToIJ($orientation, $position) {
        if (!(0 <= $orientation && $orientation <= 3)) throw new \RuntimeException();
        if (!(0 <= $position && $position <= 3)) throw new \RuntimeException();
...
// Don't instantiate
     ...
     */
    public static function robustCrossProd(S2Point $a, S2Point $b) {
        ...
        $x = S2Point::sCrossProd(S2Point::add($b, $a), S2Point::sub($b, $a));
     ...
     * = -Ortho(a) for all a.
     */
    public static function ortho(S2Point $a) {
     ...
     * are undefined
     */
    public static function robustCCW(S2Point $a, S2Point $b, S2Point $c, S2Point $aCrossB = null) {
        // assert (isUnitLength(a) && isUnitLength(b) && isUnitLength(c));
        ...
        if (is_null($aCrossB)) {
            $aCrossB = S2Point::sCrossProd($a, $b);
        }
     ...
     */
    private static function expensiveCCW(S2Point $a, S2Point $b, S2Point $c) {
        ...
        if ($dca < $dbc || ($dca == $dbc && $a->lessThan($b))) {
            if ($dab < $dbc || ($dab == $dbc && $a->lessThan($c))) {
                ...
                $sign = S2Point::sCrossProd($vab, $vca)->dotProd($a) * $sab; // BC is longest
                // edge
            } else {
                $sign = S2Point::sCrossProd($vca, $vbc)->dotProd($c) * $sca; // AB is longest
                // edge
            }
        } else {
            if ($dab < $dca || ($dab == $dca && $b->lessThan($c))) {
                $sign = S2Point::sCrossProd($vbc, $vab)->dotProd($b) * $sbc; // CA is longest
                // edge
            } else {
                $sign = S2Point::sCrossProd($vca, $vbc)->dotProd($c) * $sca; // AB is longest
                // edge
        ...
        $ccw =
            self::planarOrderedCCW(new R2Vector($a->y, $a->z), new R2Vector($b->y, $b->z), new R2Vector($c->y, $c->z));
        if ($ccw == 0) {
            $ccw =
                self::planarOrderedCCW(
    ...
    public static function planarCCW(R2Vector $a, R2Vector $b) {
    ...
    public static function planarOrderedCCW(R2Vector $a, R2Vector $b, R2Vector $c) {
        $sum = 0;
     ...
     * </ol>
     */
    public static function orderedCCW(S2Point $a, S2Point $b, S2Point $c, S2Point $o) {
