...
class S2CellId
{
    ...
    */
    ...
    public function intersects(S2CellId $other) {
        // assert (isValid() && other.isValid());
     ...
     */
    public function getEdgeNeighbors() {
        $i = 0;
        $j = 0;
        $null = null;
        $face = $this->toFaceIJOrientation($i, $j, $null);
        ...
        $neighbors[0] = $this->fromFaceIJSame($face, $i, $j - $size, $j - $size >= 0)->parent($level);
        $neighbors[1] = $this->fromFaceIJSame($face, $i + $size, $j, $i + $size < self::MAX_SIZE)->parent($level);
        $neighbors[2] = $this->fromFaceIJSame($face, $i, $j + $size, $j + $size < self::MAX_SIZE)->parent($level);
        $neighbors[3] = $this->fromFaceIJSame($face, $i - $size, $j, $i - $size >= 0)->parent($level);
     ...
     * @param int $nbrLevel
     *#/
     * public function getAllNeighbors(int $nbrLevel) {
     * $i = 0;
     * $j = 0;
     * $face = toFaceIJOrientation(i, j, null);
...
S2CellId::initLookupCell(
    ...
    0,
    0,
    S2CellId::SWAP_MASK | S2CellId::INVERT_MASK,
    0,
    S2CellId::SWAP_MASK | S2CellId::INVERT_MASK
);
