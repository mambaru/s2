...
class S1Interval
{
    ...
    public static function fromPoint(float $p) {
        if ($p == -S2::M_PI) {
            $p = S2::M_PI;
        }
     ...
     */
    public function addPoint(float $p) {
        // assert (Math.abs(p) <= S2.M_PI);
        if ($p == -S2::M_PI) {
            $p = S2::M_PI;
        }
        ...
        if ($this->isEmpty()) {
            ...
            $dlo = $this->positiveDistance($p, $this->lo());
            $dhi = $this->positiveDistance($this->hi(), $p);
            if ($dlo < $dhi) {
     ...
     * interval "y".
     */
    public function union(S1Interval $y) {
        ...
        if ($this->fastContains($y->lo())) {
            ...
            return new S1Interval($this->lo(), $y->$this->hi(), true);
        }
        if ($this->fastContains($y->hi())) {
        ...
        $dlo = $this->positiveDistance($y->hi(), $this->lo());
        $dhi = $this->positiveDistance($this->hi(), $y->lo());
        if ($dlo < $dhi) {
            return new S1Interval($y->lo(), $this->hi(), true);
        } else {
