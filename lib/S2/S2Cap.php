...
class S2Cap implements S2Region
{
     ...
     * unit-length vector.
     */
    public static function fromAxisHeight(S2Point $axis, float $height) {
     ...
     */
    public static function fromAxisArea(S2Point $axis, float $area) {
        return new S2Cap($axis, $area / (2 * M_PI));
    }
    ...
    public function complement() {
        ...
        $height = $this->isFull() ? -1 : 2 - max($this->height, 0);

        return S2Cap::fromAxisHeight(S2Point::neg($this->axis), $height);
    }
   ...
   * */
    ...
    /**
     * @param S2Cell|S2Point|S2Cap $cell
     * @return bool
     */
    public function contains($cell) {
        if ($cell instanceof S2Point) {
            return S2Point::sub($this->axis, $cell)->norm2() <= 2 * $this->height;
        } elseif ($cell instanceof S2Cell) {
        ...
        } else { // cell is instance of S2Cap
            throw new \LogicException('s2geo: This part of code was not ported propertly.');
            ...
            if ($this->isFull() || $cell->isEmpty()) {
                return true;
            }

            return $this->angle()->radians() >= $this->axis->angle($cell->axis())
                + other . angle() . radians();
        }
    }
     ...
     * alrady been checked.
     */
    public function intersects(S2Cell $cell, array $vertices) {
        ...
        $sin2Angle = $this->height * (2 - $this->height);
        for ($k = 0; $k < 4; ++$k) {
            ...
            $dir = $edge->crossProd($this->axis);
            if (
                $dir->dotProd($vertices[$k]) < 0
